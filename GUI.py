...
# This function will take a .csv file (downloaded from the community page), filter out names with special characters
# and people who were not checked in, and return the rest of names as a list.
def read_names_from_file(filename: str) -> list[str]:
    df = pd.read_csv(filename)

    # Keep only checked-in attendees
    df = df[df["Checkin Date (UTC)"].notna()]

    # Build full name
    df["Full Name"] = df["First Name"] + " " + df["Last Name"]

    # Filter by allowed characters (letters + accents + spaces + apostrophe/hyphen)
    regex_pattern = r"^[A-Za-zÀ-ÖØ-öø-ÿ\s'-]+$"
    df_filtered = df[df["Full Name"].str.contains(regex_pattern, na=False)]

    attendees_list = df_filtered["Full Name"].tolist()
    return attendees_list


def generate_certificates(filepath: str, event_title_entry: tk.Entry, event_date_entry: tk.Entry) -> None:
...
def setup_gui() -> None:
    ...
    # --- Safe logo loading (won't crash if logo.png is missing) ---
    logo_image = None
    ...
    if logo_image is not None:
        ...
        logo_label.grid(row=9, column=0, columnspan=2, sticky="ew", padx=10, pady=30)
